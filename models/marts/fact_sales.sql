WITH
    ...
    , metrics as (
        select
            ...
            , case
                when joined.weight is null then 0
                else joined.freight * (joined.order_qty * joined.weight) / weight_totals.total_weight
            end as shared_freight
            , case
                when joined.weight is null then 0
                else (joined.order_qty * joined.weight) / weight_totals.total_weight * 100.0
